...
- [ ] Write test: `test_phase_with_rich_progress`
  ...
  - Assert `self.progress_ctx is not None` (context manager created)
  - Assert `self.task_id` exists (task added to progress)
  - Mock stderr and assert it contains progress bar characters (e.g., "━" or progress output)
- [ ] Write test: `test_update_calculates_eta`
  - Simulate processing 100 items over known time period
  - Assert throughput = completed_items / elapsed_seconds
  - Assert ETA = (total - completed) / throughput (in seconds)
  - Assert ETA format matches "HH:MM:SS" (e.g., "00:03:45")
  - Assert ETA shows "calculating..." when completed < 10 items
  - Example: 50 items in 10s → throughput=5/s, 50 remaining → ETA=10s → "00:00:10"
...
- [ ] Implement Scenario 1 steps (default mode):
  ...
  - Concrete assertions:
    - `assert "→ Walking commit graph" in stderr_output`
    - `assert "→ Generating embeddings" in stderr_output`
    - `assert "→ Saving index" in stderr_output`
    - `assert re.search(r"✓ Indexing Complete\n\nStatistics:", stderr_output)`
    - `assert "Commits:" in stderr_output`
    - `assert "Unique blobs:" in stderr_output`
