...
- [ ] Implement default mode: single-line terse output (TUI_GUIDE.md line 189)
- [ ] Implement verbose mode: multi-line detailed output (TUI_GUIDE.md lines 207-236)
- [ ] Implement quiet mode: silent on success (TUI_GUIDE.md lines 240-243)
- [ ] Validate git repository exists before mock indexing
- [ ] Use `console.print()` directly, NO Rich panels/progress bars
- [ ] Enable BDD test: "Index command help"
- [ ] Add unit tests for all three output modes
...
- [ ] `gitctx index` command is executable
- [ ] Default mode outputs single terse line (TUI_GUIDE.md compliant)
- [ ] `--verbose/-v` flag shows multi-line detailed output
- [ ] `--quiet/-q` flag suppresses all output on success
- [ ] `--force/-f` flag shows "Cleared existing index" message
- [ ] Git repository validation works (checks for .git directory)
- [ ] Exit code 3 returned when not in git repository
- [ ] NO Rich panels, progress bars, or tables used
- [ ] Help text includes usage examples for all three modes
- [ ] Unit tests pass for all output modes
- [ ] BDD scenario "Index command help" passes
