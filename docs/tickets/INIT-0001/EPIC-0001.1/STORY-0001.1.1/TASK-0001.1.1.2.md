...
- [ ] Add `--verbose/-v` flag for code context display
- [ ] Implement default mode: terse result list (TUI_GUIDE.md lines 404-411)
- [ ] Implement verbose mode: results with code context (TUI_GUIDE.md lines 417-465)
- [ ] Mock results show BOTH git history AND HEAD results (demonstrate temporal search)
- [ ] Use correct format: `file:line:score ● commit (date, author) "message"`
- [ ] HEAD indicator: `●` for current HEAD, no symbol for historical commits
- [ ] NO Rich panels, tables, or syntax highlighting (save for verbose mode context)
- [ ] Enable BDD test: "Search command help"
- [ ] Add unit tests for all output modes
...
- [ ] `gitctx search <query>` command is executable
- [ ] Query argument is required (error if missing)
- [ ] Default mode: terse format `file:line:score ● commit (metadata) "message"`
- [ ] Verbose mode: multi-line with code context
- [ ] `--limit/-n` option controls result count
- [ ] Mock results demonstrate BOTH git history AND HEAD
- [ ] HEAD indicator: `●` for HEAD commits, space for historical
- [ ] NO Rich panels, tables, or Syntax highlighting (plain console.print)
- [ ] Help text includes usage examples
- [ ] Unit tests pass for all output modes
- [ ] BDD scenario "Search command help" passes
