...
1. **`src/gitctx/storage/__init__.py`**
   ...
   __all__ = ["LanceDBStore", "VectorStoreProtocol", "CodeChunkRecord", "CHUNK_SCHEMA"]
   ```
...
5. **`tests/unit/storage/conftest.py`**
   - Fixtures for test LanceDB instances
   - Mock `Embedding` factory
   - Mock `BlobLocation` factory
...
Update `tests/e2e/steps/lancedb_storage_steps.py`:
...
- ✅ All 14 unit tests pass (TDD workflow followed)
