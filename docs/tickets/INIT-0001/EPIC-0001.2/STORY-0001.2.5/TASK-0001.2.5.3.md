...
- [ ] Implement `_count_lines(self, repo_path: Path) -> int`:
  ...
  - Filter: `.py, .js, .ts, .go, .rs` files only
  - Exclude: `.git` in file.parts
  - For each file: `len(file.read_text().splitlines())`
  - Catch UnicodeDecodeError, PermissionError: continue (skip binary/inaccessible)
  - Return sum of lines across all files
- [ ] Implement `_count_files(self, repo_path: Path) -> int`:
  - Same filtering as _count_lines()
