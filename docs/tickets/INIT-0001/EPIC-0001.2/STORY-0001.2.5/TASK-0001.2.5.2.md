...
- [ ] Implement `start(self)`:
  ...
  - If not verbose: prepare spinner (show after 5s in update loop)
...
- [ ] Implement `update(self, commits=0, blobs=0, chunks=0, tokens=0, cost=0.0)`:
  - Update stats fields
  - If not verbose and elapsed >5s: show spinner "⠋ Indexing... 8.3s" (update every 0.1s)
...
- [ ] Implement `@then('I should see phase markers: {markers}')`:
  - Parse stderr for phase markers
  - Assert "→ Walking commit graph" in stderr
  - Assert "→ Generating embeddings" in stderr
- [ ] Implement `@then('final summary should show detailed statistics table')`:
  - Parse stderr for "✓ Indexing Complete"
  - Assert statistics table present: "Commits:", "Unique blobs:", "Tokens:", "Cost:"
