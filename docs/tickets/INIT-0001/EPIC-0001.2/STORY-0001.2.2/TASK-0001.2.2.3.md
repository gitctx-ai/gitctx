...
- [ ] Write unit tests in `tests/unit/core/test_chunker.py`
  ...
  - [ ] Test `chunk_file` with content < max_tokens → single chunk
  - [ ] Test `chunk_file` with content > max_tokens → multiple chunks
  - [ ] Test `chunk_file` with empty string → empty list
...
- [ ] Write token ratio validation tests in `tests/unit/core/test_token_ratio.py`
  ...
  - [ ] Verify every chunk has token_count ≤ 880 (max_tokens * 1.1)
...
- [ ] Implement BDD steps in `tests/e2e/steps/test_chunking.py`
  ...
  - [ ] Scenario 1: "Chunk large Python file with overlap"
    ...
    - [ ] @then: Verify 7-8 chunks, each ≤800 tokens, ~20% overlap, line metadata
  ...
  - [ ] Scenario 9: "Token limit compliance verification"
    ...
    - [ ] @then: Verify 100% chunks ≤880 tokens (10% tolerance)
- [ ] Run BDD scenarios - Scenarios 1-6, 8-9 should pass ✓
...
- This is the core implementation task - most complex in the story
